...
class trainADS(nn.Module):
    def __init__(self, args, logger):
        ...
        if os.path.isfile(f"{self.args.data_dir}/adswoIwCtrain.pickle"):
            logger.info('***** Loading Train Data from pickle *****')
            with gzip.open(f"{self.args.data_dir}/adswoIwCtrain.pickle", 'rb') as f:
                train_data = pickle.load(f)
        else:
            train_data = get_scoring_dataset(self.tokenizer, self.dialog_model, "train", self.args)
            with gzip.open(f"{self.args.data_dir}/adswoIwCtrain.pickle", 'wb') as f:
                pickle.dump(train_data, f)
        if os.path.isfile(f"{self.args.data_dir}/adswoIwCvalid.pickle"):
            logger.info('***** Loading Valid Data from pickle *****')
            with gzip.open(f"{self.args.data_dir}/adswoIwCvalid.pickle", 'rb') as f:
                valid_data = pickle.load(f)
        else:
            valid_data = get_scoring_dataset(self.tokenizer, self.dialog_model, "valid", self.args)
            with gzip.open(f"{self.args.data_dir}/adswoIwCvalid.pickle", 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_name', type=str, default='best_ckpt_epoch=5_valid_loss=0.0135', help="The name of the trained checkpoint. (without extension)")
